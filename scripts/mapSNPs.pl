...
#open (AVG, ">$snpfile.avglink" ) or die "can't open file to write: $!\n";
...
# remove the error positions from the link matrix
#removeErr();
...
#		if ($index1 == 81 or $index2 == 81) {
#			print "indexes shared by read: rid: $i: @{ $snpreads->{$i} }\n";
#			$flag == 1;
#				print "$index1, $index2: $snp_pairs[$index1][$index2]\n"
#		}
			}
		}
#		die if $flag == 1;
...
# removeErr
#
# remove the snps whose indices are in @errpos, also remove
# the read-mapping counts that these error positions share with 
# non-error positions
######################
# End of removeErr #
